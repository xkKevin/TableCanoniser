...
for row in messy_data:
    if row[0]:  # If the first element of the row is not empty (indicating new device or category)
        if row[1].startswith('$'):  # This row contains device name and price
            ...
            category = row[0]
            if category.endswith("Date"):
                current_release_date = row[1]
            elif category == 'Dimensions':
                # Update dimensions based on identifiers 'H', 'W', 'D'
                for i in range(1, len(row), 2):
                    if row[i] in ['H', 'Height']:
                        current_dimensions['Height'] = row[i+1]
                    elif row[i] in ['W', 'Width']:
                        current_dimensions['Width'] = row[i+1]
                    elif row[i] in ['D', 'Depth']:
                        current_dimensions['Depth'] = row[i+1]
            elif category == 'Weight':
                current_weight = row[1]
            elif category == 'Camera':
                # Ensure Front Camera is always smaller than Rear Camera
